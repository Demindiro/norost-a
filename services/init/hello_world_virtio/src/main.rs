...
const PLATFORM_INFO: NonNull<kernel::Page> = unsafe { NonNull::new_unchecked(0x10_0000_0000 as *mut _) };
const PCI_ADDRESS: NonNull<kernel::Page> = unsafe { NonNull::new_unchecked(0x20_0000_0000 as *mut _) };
const PCI_ADDRESS_MMIO: NonNull<kernel::Page> = unsafe { NonNull::new_unchecked(0x30_0000_0000 as *mut _) };
...
fn dump_dtb(dtb: &device_tree::DeviceTree) {
	...
	for rmr in dtb.reserved_memory_regions() {
		...
		writeln!(kernel::SysLog, "  {:x} <-> {:x} (len: {:x})", addr, addr + size, size);
	}
	...
	fn print_node(level: usize, mut node: device_tree::Node) {
		...
		while let Some(property) = node.next_property() {
				...
				property.value.last().unwrap() == &0
			{
				...
				writeln!(kernel::SysLog, "{0:>1$}{2} = {3:?}", "", level * 2 + 2, property.name, s);
			} else {
				writeln!(kernel::SysLog, "{0:>1$}{2} = {3:02x?}", "", level * 2 + 2, property.name, &property.value);
			}
		}
		...
		while let Some(mut node) = node.next_child_node() {
			if node.name == "soc" {
				...
				while let Some(mut node) = node.next_child_node() {
					...
					unsafe { kernel::sys_direct_alloc(addr.as_ptr(), start >> 12, size >> 12, 0b111); }

					// Map MMIO
					let addr = PCI_ADDRESS_MMIO;
					let ranges = &ranges.unwrap()[(child_address_cells + address_cells + size_cells) as usize * 4..];
...
fn main() {
	let kernel::Return { status, value } = unsafe {
		kernel::sys_platform_info(
			PLATFORM_INFO.as_ptr(),
			256,
		)
	};
		...
		if dev.vendor_id() == VIRTIO_VENDOR_ID {
			...
			match dev.device_id() {
				VIRTIO_DEV_BLK_ID => {
					...
					for (s, d) in b"Hello, world! I am a userland driver!\nLook at me, I parse the FDT now!".iter().copied().zip(data.iter_mut()) {
