...
impl<'a> Queue<'a> {
	...
	pub fn new(config: &'a super::pci::CommonConfig, index: u16, max_size: u16) -> Result<Self, OutOfMemory> {
		...
		let avail_size = mem::size_of::<AvailHead>() + mem::size_of::<AvailElement>() * size
			+ mem::size_of::<AvailTail>();
		let used_size = mem::size_of::<UsedHead>() + mem::size_of::<UsedElement>() * size
		...
		let ret = unsafe { kernel::dev_dma_alloc(DMA_ADDR as *mut kernel::Page, align(desc_size + avail_size) + align(used_size), 0x2) };
		...
		let used = unsafe { NonNull::<Used>::new_unchecked(mem.add(align(desc_size + avail_size)).cast()) };
		...
		I: ExactSizeIterator<Item = (usize, usize, bool)>,
	{
		...
		unsafe {
			...
			let desc: &mut [Descriptor] = slice::from_raw_parts_mut(self.descriptors.as_ptr(), size);
			let avail_head = &mut *self.available.as_ptr().cast::<AvailHead>();
			let avail_ring = self.available.as_ptr().cast::<u8>().add(mem::size_of::<AvailHead>());
			let avail_ring: &mut [u16] = slice::from_raw_parts_mut(avail_ring.cast(), size);
			let used_head = &mut *self.used.as_ptr().cast::<UsedHead>();
			let used_ring = self.used.as_ptr().cast::<u8>().add(mem::size_of::<UsedHead>());
			...
			while let Some((address, length, write)) = iterator.next() {
				...
				*prev_next = u16le::from(i as u16);
				writeln!(kernel::SysLog, "length {} | flags 0b{:b} | next {}", desc[i].length, desc[i].flags, *prev_next);
