...
A: Allocator,
{
	pub fn new(common: &'a CommonConfig, device: &'a DeviceConfig, notify: &'a Notify) -> Result<Self, SetupError> {
		...
		common.device_status.set(CommonConfig::STATUS_ACKNOWLEDGE
	...
	pub fn write(&mut self, data: &[u8], sector_start: u64) -> Result<(), WriteError> {
		...
		let ret = unsafe { kernel::mem_physical_address(hp as *const _, &mut phys_header as *mut _, 1) };
		assert_eq!(ret.status, 0, "Failed DMA get phys address");
		let ret = unsafe { kernel::mem_physical_address(dp as *const _, &mut phys_data as *mut _, 1) };
		assert_eq!(ret.status, 0, "Failed DMA get phys address");
		let ret = unsafe { kernel::mem_physical_address(sp as *const _, &mut phys_status as *mut _, 1) };
		assert_eq!(ret.status, 0, "Failed DMA get phys address");
		...
		self.queue.send(data.iter().copied()).expect("Failed to send data");
